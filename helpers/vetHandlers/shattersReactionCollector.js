...
async function shattersReactionCollector(cli, cfg, reactionCollector){
	...
	reactionCollector.on('collect', async (reaction, user) => {
		...
		if (reaction.emoji.id == '679186891463196673'){ // key
			...
			if (reaction.count == 3){
				cli.channels.cache.get(cfg.fungalcavern.vetRlCommands).send(`${user} tried to react with ${cli.emojis.cache.find(r => r.id === reaction.emoji.id)} but there is already someone.`);
	...
	reactionCollector.on('remove', async (reaction, user) => {
		...
		if (reaction.emoji.id == '679186891463196673'){ // key
			if (afkObj['key'] == user.id){
				...
				cli.channels.cache.get(cfg.fungalcavern.vetRlCommands).send(`${user} unreacted from being the main ${cli.emojis.cache.find(e => e.id == "679186891463196673")}.`);
