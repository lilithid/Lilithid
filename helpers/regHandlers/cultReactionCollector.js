	...
	reactionCollector.on('collect', async (reaction, user) => {
		...
		if (reaction.emoji.id == '686217948507275374'){ // key
			...
			if (await powerCalculation(cli, cfg, user.id) < 2){
				cli.channels.get(cfg.fungalcavern.rlcommands).send(`${user} tried to react with stop but isn't allowed to.`);
	...
	reactionCollector.on('remove', async (reaction, user) => {
		...
		if (reaction.emoji.id == '686217948507275374'){ // key
			...
			if (afkObj['rusher'].includes(user.id)){
				...
				cli.channels.cache.get(cfg.fungalcavern.rlcommands).send(`${user} unreacted from being ${cli.emojis.cache.find(e => e.id == "686222430259380427")}.`);
			}
		} else if (reaction.emoji.id == '679186994190090270'){ // warrior
