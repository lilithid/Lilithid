...
async function cultReactionCollector(cli, cfg, reactionCollector){
	...
	reactionCollector.on('collect', async (reaction, user) => {
		...
		var cpMsg;
		var afkMsg;
		cpMsg = await cli.channels.cache.find(chan => chan.id == cfg.fungalcavern.rlcommands).messages.fetch(afkObj['controlpanel']);
		afkMsg = await cli.channels.cache.find(chan => chan.id == cfg.fungalcavern.afkchecks).messages.fetch(afkObj['afkcheck']);
		//
		...
		if (reaction.emoji.id == '760021357076938782'){ // key
			if (afkObj['key'] == undefined){
				const keyMsg = await user.send(`You have reacted with ${cli.emojis.cache.find(e => e.id === "760021357076938782")}.\nIf you actually have a key, react with ✅ and if you made a mistake, ignore this message.`).catch(() => cpMsg.channel.send(`${user} tried to react with key but I couldn't pm.`));
				await keyMsg.react('✅');
			...
			if (await powerCalculation(cli, cfg, user.id) < 2){
				//cli.channels.cache.get(cfg.fungalcavern.rlcommands).send(`${user} tried to react with stop but isn't allowed to.`);
			...
			if (!afkObj['postafk']){
                ...
                // MOVE OUT
                var reactedPortal = [];
                var reactIDS = await afkMsg.reactions.cache.get('760021346871803924').users.fetch();
                for (var i of reactIDS){ reactedPortal.push(i[0]); }
                await cli.channels.cache.get(afkObj['channel']).members.forEach(async function(raiders){ if (!reactedPortal.includes(raiders.user.id) && await powerCalculation(cli, cfg, raiders.user.id) < 2) await cli.guilds.cache.get(cfg.fungalcavern.id).members.cache.get(raiders.user.id).voice.setChannel(cli.channels.cache.get('635551563443863583')).catch(err => console.error(err)); }) // Afk channel


				// lock the channel
				await cli.channels.cache.get(afkObj['channel']).createOverwrite(cfg.fungalcavern.raiderRole, { 'VIEW_CHANNEL': true, 'CONNECT': false, 'SPEAK': false });
   				await cli.channels.cache.get(afkObj['channel']).setName(cfg.fungalcavern.vc[afkObj['channelNumber']].name);
			}
			if (afkObj['postafk']){
				if (reaction.message.reactions.cache.get('760024527971024927') != undefined) await reaction.message.reactions.cache.get('760024527971024927').remove().catch(error => console.error('Failed to remove reactions: ', error));
	...
	reactionCollector.on('remove', async (reaction, user) => {
		...
		if (reaction.emoji.id == '760021357076938782'){ // key
			if (afkObj['key'] == user.id){
				...
				cli.channels.cache.get(cfg.fungalcavern.rlcommands).send(`${user} unreacted from being the main ${cli.emojis.cache.find(e => e.id == "686217948507275374")}.`);
			...
			if (afkObj['rusher'].includes(user.id)){
				...
				cli.channels.cache.get(cfg.fungalcavern.rlcommands).send(`${user} unreacted from being ${cli.emojis.cache.find(e => e.id == "760021417391030274")}.`);
			}
		} else if (reaction.emoji.id == '760021468863266817'){ // warrior
