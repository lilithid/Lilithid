async function updateNormalAfkEmbed(cli, cfg, afkObj){
	...
	const embed = {
		description: `We are starting an afk check now, join \`${channelName}\` and react with ${eventReact} to not get moved out! If you react with keys or classes and do not bring them, you may be suspended. **Make sure to react with the classes you're bringing!**\n\nStarting in ${((afkObj['timeleft']/60).toString()).charAt(0)} minutes and ${(afkObj['timeleft']%60)} seconds! In addition to reacting with ${eventReact} also react...`,
		color: `${afkObj['color']}`,
...
/*		fields: [
		{
			...
			value: `${cli.emojis.cache.find(e => e.id == afkObj['eventPortalID'])}`,
			inline: true
		},
		{
