...
async function shattersReactionCollector(cli, cfg, reactionCollector){
	...
	reactionCollector.on('collect', async (reaction, user) => {
		...
		if (reaction.emoji.id == '679186891463196673'){ // key
			...
			if (reaction.count == 3){
				cli.channels.cache.get(cfg.fungalcavern.eventRlCommands).send(`${user} tried to react with ${cli.emojis.cache.find(r => r.id === reaction.emoji.id)} but there is already someone.`);
			...
			if (await powerCalculation(cli, cfg, user.id) < 2){
				cli.channels.get(cfg.eventRlCommands).send(`${user} tried to react with stop but isn't allowed to.`);
	...
	reactionCollector.on('remove', async (reaction, user) => {
		...
		if (reaction.emoji.id == '679186891463196673'){ // key
			if (afkObj['key'] == user.id){
				...
				cli.channels.cache.get(cfg.fungalcavern.eventRlCommands).send(`${user} unreacted from being the main ${cli.emojis.cache.find(e => e.id == "679186891463196673")}.`);
