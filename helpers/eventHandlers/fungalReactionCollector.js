...
async function cultReactionCollector(cli, cfg, reactionCollector){
	...
	reactionCollector.on('collect', async (reaction, user) => {
		...
		if (reaction.emoji.id == '686223695798075449'){ // key
		...
		} else if (reaction.emoji.id == '679187016071512067'){ // priest
			if (await isInOtherClasses(user.id, afkObj)){
				await user.send(`Sorry but you cannot react with multiples classes at the same time.`);
				let reactClass = reaction.message.reactions.cache.get('679187016071512067');
				if (reactClass != undefined) await reactClass.users.remove(user).catch(error => console.error(err));
		...
		} else if (reaction.emoji.id == '686224526123728969'){ // trickster
			if (await isInOtherClasses(user.id, afkObj)){
				await user.send(`Sorry but you cannot react with multiples classes at the same time.`);
				let reactClass = reaction.message.reactions.cache.get('686224526123728969');
				if (reactClass != undefined) await reactClass.users.remove(user).catch(error => console.error(err));
				return true;
			} else {
			...
			if (await powerCalculation(cli, cfg, user.id) < 2){
				cli.channels.get(cfg.fungalcavern.eventRlCommands).send(`${user} tried to react with stop but isn't allowed to.`);
	...
	reactionCollector.on('remove', async (reaction, user) => {
		...
		var afkMsg;
		cpMsg = await cli.channels.cache.find(chan => chan.id == cfg.fungalcavern.eventRlCommands).messages.fetch(afkObj['controlpanel']);
		afkMsg = await cli.channels.cache.find(chan => chan.id == cfg.).messages.fetch(afkObj['afkcheck']);
		//
		...
		if (reaction.emoji.id == '686223695798075449'){ // key
			if (afkObj['key'] == user.id){
				...
				cli.channels.cache.get(cfg.fungalcavern.eventRlCommands).send(`${user} unreacted from being the main ${cli.emojis.cache.find(e => e.id == "686217948507275374")}.`);
			...
			if (afkObj['rusher'].includes(user.id)){
				...
				cli.channels.cache.get(cfg.fungalcavern.eventRlCommands).send(`${user} unreacted from being ${cli.emojis.cache.find(e => e.id == "686222430259380427")}.`);
			}
		} else if (reaction.emoji.id == '679186994190090270'){ // warrior
