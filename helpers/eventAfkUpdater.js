...
async function eventAfkUpdater(cli, cfg){
	// update all afks
	await eventAfkArray.forEach(async afk => {
		const afkMsg = await cli.channels.cache.find(chan => chan.id == cfg.fungalcavern.eventAfkchecks).messages.fetch(afk['afkcheck']);
		const cpMsg = await cli.channels.cache.find(chan => chan.id == cfg.fungalcavern.eventRlCommands).messages.fetch(afk['controlpanel']);
		...
		if (afk['type'] == 'event') directoryEmbed = `eventEmbeds`;
		...
		if (afk['type'] == 'event') portalReact = afk['eventPortalID']; // event portal
			...
			if (!afk['ended']){
				if (afk['timeleft'] <= 0){
					if (!afk['postafk']){
						...
						await cli.channels.cache.get(afk['channel']).members.forEach(async function(raiders){ if (!reactedPortal.includes(raiders.user.id) && await powerCalculation(cli, cfg, raiders.user.id) < 2) await cli.guilds.cache.get(cfg.fungalcavern.id).members.cache.get(raiders.user.id).voice.setChannel(cli.channels.cache.get(cfg.fungalcavern.eventVc[0].id)).catch(err => console.error(err)); }) // Vet Lounge

						// locks the channel
						await cli.channels.cache.get(afk['channel']).createOverwrite(cfg.fungalcavern.eventRaiderRole, { 'VIEW_CHANNEL': true, 'CONNECT': false, 'SPEAK': false });
   						await cli.channels.cache.get(afk['channel']).setName(cfg.fungalcavern.eventVc[afk['channelNumber']].name);
					} else {
						// post afk -> ended
						if (afkMsg.reactions.cache.get('760024527971024927') != undefined) await afkMsg.reactions.cache.get('760024527971024927').remove().catch(error => console.error('Failed to remove reactions: ', error));
						afk['ended'] = true;
				...
				var reactIDS = await afkMsg.reactions.cache.get(portalReact).users.fetch();
				for (var i of reactIDS){ dbrunDone(i[0], 'e') }
				if (afk['key']) await dbpopKey(afk['key'].id, '1', 'e', function(result){}).catch(err => console.error(err));

				let index = eventAfkArray.indexOf(afk);
				if (index > -1) {
  					eventAfkArray.splice(index, 1);
				}
...
module.exports = eventAfkUpdater;
module.exports.eventAfkArray = eventAfkArray;