async function lock(cli, cfg, data){
    ...
    await cli.channels.cache.get(cfg.fungalcavern.vetVc[args[1]].id).createOverwrite(cfg.fungalcavern.vetRaiderRole, { 'VIEW_CHANNEL': true, 'CONNECT': false, 'SPEAK': false });
    await cli.channels.cache.get(cfg.fungalcavern.vetVc[args[1]].id).setName(cfg.fungalcavern.vetVc[args[1]].name);
    await data.channel.send(`Locked channel ${cfg.fungalcavern.vetVc[args[1]].name.toLowerCase()}`);
}
