...
const { remove_user, findbyIGN } = require('../../db.js');

async function unverify(cli, cfg, data){
	...
	await findbyIGN(user, async function(result){
		if (result == undefined){
			...
			// log mod-mails
			const embed = {
				"title": `User ${user} was unverified.`,
				"fields": [
					{
						"name":"Mods name",
						"value":`${data.author}`,
			...
			await remove_user(user, args[2]);
			await cli.guilds.cache.get(cfg.fungalcavern.id).members.cache.get(user).roles.remove(cli.guilds.cache.get(cfg.fungalcavern.id).roles.cache.find(r => r.name === "Verified Raider").id).catch(console.error);
			await cli.guilds.cache.get(cfg.fungalcavern.id).members.cache.get(user).setNickname('', `Reason: Unverification.`);
			data.channel.send(`User was succesfuly unverified!`);
		}
	})
}
