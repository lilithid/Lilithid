...
async function vetUnverify(cli, cfg, data){
	...
	await findbyIGN(user, async function(result){
		if (result == undefined){
			...
			user = result.id;
			// profile not private
			await continueVetUnverify(cli, cfg, data, args, user);
		}
	})

}

async function continueVetUnverify(cli, cfg, data, args, user){
	// proofs or not
	let proofs;
	...
	// log mod-mails
	const embed = {
		"title": `Veteran removed for user ${args[1]}`,
		"fields": [
			{
				"name":"Mods name",
				"value":`${data.author}`,
