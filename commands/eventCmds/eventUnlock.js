async function vetlock(cli, cfg, data){
    ...
    await cli.channels.cache.get(cfg.fungalcavern.eventVc[args[1]].id).createOverwrite(cfg.fungalcavern.eventRaiderRole, { 'VIEW_CHANNEL': true, 'CONNECT': true, 'SPEAK': false });
    await cli.channels.cache.get(cfg.fungalcavern.eventVc[args[1]].id).setName(`${cfg.fungalcavern.eventVc[args[1]].name} <-- Join!`);
    await data.channel.send(`Unlocked channel ${cfg.fungalcavern.eventVc[args[1]].name.toLowerCase()}`);
}
